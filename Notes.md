...
2. Retry Logic: If for some reason OCR / LLMs fail with an error response then we should add a retry logic where the service tries to retry calling the API thrice before returning error response to the backend and then to frontend showing the user the services are temporarily down.
